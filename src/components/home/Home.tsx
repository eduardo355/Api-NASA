...
const Home = () => {
    ...
    const [apo, setApo] = useState<typesGetNeo[]>([])
    ...
    if(error) {
        return <h1 className=" text-center font-bold text-2xl text-balance">Ha ocurrido un error, intentalo mas tarde.</h1>
    }
                        ...
                        return (
                            ...
                            >
                                <div className=" flex flex-col h-52">
                                    ...
                                    <span className="text-gray-400"><b className="text-white">Designacion:</b> {neo.designation}</span>
                                    <span className="text-gray-400"><b className="text-white">Magnitud absoluta:</b> {neo.absolute_magnitude_h}</span>
                                    <span className="text-white text-xl"><b>Diametro:</b></span>
                                    <div className=" flex flex-col">
                                        <span className="text-gray-400"><b className="text-white">Maximo:</b> {neo.estimated_diameter.kilometers.estimated_diameter_max}</span>
                                        <span className="text-gray-400"><b className="text-white">Minimo:</b> {neo.estimated_diameter.kilometers.estimated_diameter_min}</span>
                                    </div>
                                    <span className="text-gray-400"><b className="text-white">Potencialmente peligroso:</b> {neo.is_potentially_hazardous_asteroid ? 'Si' : 'No'}</span>
                                    <img className="absolute left-52 top-20 opacity-40" src="./asteroide.png" alt="" width={90} />
                                </div>
                                <button
                                    onClick={() => navegacion(`/Asteroide/${neo.id}`)}
                                    className={` ${typeAsteroid ? 'bg-red-400 hover:bg-red-500' : 'bg-indigo-400 hover:bg-indigo-500'} bg-indigo-300 text-gray-100 mt-6 rounded p-2 px-6`}
                                >
                                    Ver mas
                                </button>
                            </div>
